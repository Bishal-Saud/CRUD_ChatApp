...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (uniqueMembers) return next(new ErrorHandler("Member already exits"));
...
const removeMember = TryCatch(async (req, res, next) => {
  ...
  if (chat.members.length <= 3)
    return next(new ErrorHandler("Group must at least", 400));
...
const leaveGroup = TryCatch(async (req, res, next) => {
  try {
    ...
    emitEvent(req, chat.members, `User ${user} has left the group`);
